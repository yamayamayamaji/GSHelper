...
//      var locPath = location.pathname;
    ...
    versionMgr: {
        ...
        notifyUpdate: function(){
            ...
            var anc = $('<a href="' + path + '" target="_blank">show details</a>')
                        .addClass('sc_ttl_sat');
    ...
    setupInBrowserViewBtn: function(fileLinks, handler, opt){
        ...
        $fileLinks.each(function(){
            ...
            $('<span class="' + btnCls + '" title="ブラウザで開く"></span>')
            .css({ display: opt.disp })
            .insertAfter($flink);
        });

        $(document.body)
        .off('click', btnSelector)
        .on('click', btnSelector, handler);
    },
    ...
    bbs060: {
        ...
        init: function(){
            var self = this,
                btnSelector = '.open-in-browser',
                rootSelector = '#post_list_block',
                targetElmSelector = '.menu_bun a',
                targetAreaSelector = rootSelector + ' #post_list table td:has(' + targetElmSelector + ')';

            $(document.body).on({
                mouseover: function() {
                    //インブラウザビューボタン作成
                    GSHelper.setupInBrowserViewBtn(
                        $(targetElmSelector, this),
                        self.clickInBrowserViewBtn.bind(self)
                    );
                }
            }, targetAreaSelector + ':not(:has(' + btnSelector + '))');

            //インブラウザビューボタン表示エフェクト
            $(document.body).on({
                mouseenter: function(){
                    $(this).find(btnSelector).css('display', 'inline-block');
                },
                mouseleave: function(){
                    $(this).find(btnSelector).css('display', '');
                }
            }, targetAreaSelector);
        },
    ...
    sml010: {
        ...
        init: function(){
            var self = this,
                body = document.body,
                btnSelector = '.open-in-browser',
                targetElmSelector = 'a[onclick*=fileLinkClick]',
                targetAreaSelector = '#mail_kakunin_body_tr table td:has(' + targetElmSelector + ')';

            //インブラウザビューボタン作成
            $(body).on('mouseover', targetAreaSelector + ':not(:has(' + btnSelector + '))', function() {
                GSHelper.setupInBrowserViewBtn(
                    $(targetElmSelector, this),
                    self.clickInBrowserViewBtn.bind(self)
                );
            ...
            //返信時の件名を短くする
            $(body).on('click', '.head_menu_replay_btn, .head_menu_all_replay_btn', function() {
                var id = setInterval(function() {
                    var $title = $('input[name=sml020Title]:visible');
                    if ($title.length && $title.val() != '') {
                        self.shortenReplyTitle();
                        clearInterval(id);
                    }        
                }, 300);
            });
        ...
        clickInBrowserViewBtn: function(evt){
            var $btn = $(evt.target),
                ...
                reqUrl = '../smail/sml030.do?' + $.param(data);
